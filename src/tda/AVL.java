...
public class AVL<E> { // AVL
    ...
    public AVL(final Comparator<E> f) {
        this.f = f;
        this.root = null;
    }
    ...
    public void setRoot(final Nodo<E> root) {
        this.root = root;
    }
     ...
     * @return int
     */
    private int height(final Nodo<E> n) {
        if (n == null)
            return 0;
    ...
    private int totalNodos(final Nodo<E> n) {
        if (n == null)
            return 0;
    ...
    private int contarHojas(final Nodo<E> n) {
        if (n == null)
            return 0;
    ...
    private Nodo<E> add(final E element, Nodo<E> n) {
        if (n == null) {
            n = new Nodo<>(element);
    ...
    private Nodo<E> insert(final E element, Nodo<E> n) {
        if (n == null) {
            n = new Nodo<>(element);
            n.setFe(0);
        } else if (f.compare(element, n.getData()) >= 0) {
    ...
    private boolean esDegenerado(final Nodo<E> p) {
        if (p == null) {
            return true;
    ...
    /**
     * Calcula el Factor de Equilibrio: FE mide la carga del nodo. Puede ser 0, 1,
     * 2, -1, -2
     * 
     * @return int
     */
    ...
    private int calcularCarga(final Nodo<E> n) {
        if (n == null)
            return -1;
        final int Carga = height(n.getRight()) - height(n.getLeft());
        return Carga;
    }

    /**
     * Realiza la operacion de rotacion simple derecha en el subarbol . Se ingresa
     * el nodo que tiene carga 2
     * 
     * @param nodo raiz del subarbol a rotar.
     * @return Nodo que pasa a ser la raiz del subarbol estructurado.
     */
    public Nodo<E> rotacionSimpleIzquierda(final Nodo<E> nodo) {
        if (nodo == null)
            return null;
        Nodo<E> nodoTemp;

        final Nodo<E> nodoPapa = nodo.getRight();
        nodo.setRight(null);
    ...
    /**
     * Realiza la operacion de rotacion simple izquierda en el subarbol que tiene
     ...
     */
    public Nodo<E> rotacionSimpleDerecha(final Nodo<E> nodo) {
        if (nodo == null)
            return null;
        Nodo<E> nodoTemp;

        final Nodo<E> nodoPapa = nodo.getLeft();
        nodo.setLeft(null);
    ...
    public Nodo<E> rotacionDobleIzquierda(final Nodo<E> nodo) {
        if (nodo == null)
            return null;
        Nodo<E> nodoTemp;

        final Nodo<E> nodoPapa = nodo.getLeft().getRight();
    ...
    public Nodo<E> rotacionDobleDerecha(final Nodo<E> nodo) {
        if (nodo == null)
            return null;
        Nodo<E> nodoTemp;

        final Nodo<E> nodoPapa = nodo.getRight().getLeft();
    ...
    private boolean contains(final E element, final Nodo<E> n) {
        if (n == null)
            return false;
        else if (f.compare(element, n.getData()) == 0)
            return true;
    ...
    private void posOrden(final Nodo<E> n) {
        if (n != null) {
            posOrden(n.getLeft());
            posOrden(n.getRight());
    ...
    private void preOrden(final Nodo<E> n) {
        if (n != null) {
            System.out.println(n.getData());
            preOrden(n.getLeft());
            preOrden(n.getRight());
    ...
    private void enOrden(final Nodo<E> n) {
        if (n != null) {
            enOrden(n.getLeft());
            System.out.println(n.getData());
            enOrden(n.getRight());
    ...
    public int nivel(final E element) {
        if (element == null || isEmpty() || this.contains(element))
            return -1;
        return nivel(element, root);
    }

    private int nivel(final E element, final Nodo<E> n) {
        final int num = height() - 1;
        if (n == null)
            return -1;
        else if (f.compare(element, n.getData()) == 0) {
            return num;
    ...
    private boolean equals(final Nodo<E> n1, final Nodo<E> n2) {
        if (n1 == null && n2 == null)
            return true;
    ...
    public Pane mostrarArbol() {
        final Pane vbArbol = new Pane();
        final Pane lineas = new Pane();

        mostrarArbol(vbArbol, root, 0);
        for (int i = 0; i < height(); i++) {
            final HBox hb = (HBox) vbArbol.getChildren().get(i);
            ...
            if (i != height() - 1) {
                for (int j = 0; j < hb.getChildren().size(); j++) {
                    ...
                    final StackPane st = (StackPane) hb.getChildren().get(j);
                    if (((Circle) st.getChildren().get(0)).getOpacity() != 0) {
                        final Label lbl = (Label) st.getChildren().get(1);
                        final Circle cir = (Circle) st.getChildren().get(0);
                        st.getChildren().clear();
                        final HBox hbNext = (HBox) vbArbol.getChildren().get(i + 1);
                        final StackPane stNextIz = (StackPane) hbNext.getChildren().get(j * 2);
                        final StackPane stNextDer = (StackPane) hbNext.getChildren().get(j * 2 + 1);

                        if (((Circle) stNextIz.getChildren().get(0)).getOpacity() != 0) {
                            final Path path = new Path(new PathElement[] {
                                    new MoveTo(20 + 35 * (Math.pow(2, ((height() - i) - 1)) - 1)
                                            + j * (60 + 60 * (Math.pow(2, ((height() - i - 1))) - 1)), 20 + i * 70),
                                    new LineTo(
                                            20 + 35 * (Math.pow(2, ((height() - i) - 2)) - 1)
                                                    + j * 2 * (60 + 60 * (Math.pow(2, ((height() - i - 2))) - 1)),
                                            90 + i * 70) });
                        ...
                        if (((Circle) stNextDer.getChildren().get(0)).getOpacity() != 0) {
                            final Path path = new Path(new PathElement[] {
                                    ...
                                    new LineTo(
                                            20 + 35 * (Math.pow(2, ((height() - i) - 2)) - 1)
                                                    + (j * 2 + 1) * (60 + 60 * (Math.pow(2, ((height() - i - 2))) - 1)),
                                            90 + i * 70) });
                            lineas.getChildren().add(path);
                        }
    ...
    private void mostrarArbol(final Pane vb, final Nodo<E> n, final int nivel) {
        ...
        if (idexOutOfBoundsException(vb, nivel)) {
            ...
            hb = (HBox) vb.getChildren().get(nivel);
    ...
    private boolean idexOutOfBoundsException(final Pane vb, final int n) {
        try {
    ...
    public boolean delete(final Object o) throws ClassCastException, NullPointerException {
        ...
        @SuppressWarnings("unchecked")
        final E a = (E) o;
        if (this.compararDato(a, raizTmp.getData()) == 0) {
            salir = true;
            borrar = raizTmp;
        }
        ...
        while (!salir && (raizTmp.getRight() != null || raizTmp.getLeft() != null)) {
            ...
            } else if (this.compararDato(a, raizTmp.getData()) < 0) {
        ...
        if (salir) {
            ...
            if (borrar.getLeft() == null && borrar.getRight() == null) {
                ...
                if (nPadre.getLeft() != null && compararDato(nPadre.getLeft().getData(), borrar.getData()) == 0) {
                    nPadre.setLeft(null);
                } else if (nPadre.getRight() != null
                        && compararDato(nPadre.getRight().getData(), borrar.getData()) == 0) {
            ...
            // Nodo no hoja
                ...
                final Nodo<E> papa = padre(cambiar);
            ...
            while (equilibrado(this.getRoot()) < 0) {
                ...
                if (compararDato(superior.getData(), mirar.getData()) != 0) {
                    if (superior.getLeft() != null
                            && compararDato(superior.getLeft().getData(), mirar.getData()) == 0) {
                        superior.setLeft(cambiar2);
                    } else if (superior.getRight() != null
                            && compararDato(superior.getRight().getData(), mirar.getData()) == 0) {
    ...
    private int compararDato(final E t1, final E t2) {
        if (this.f == null) {
            @SuppressWarnings("unchecked")
            final Comparable<E> tt1 = (Comparable<E>) t1;
            return tt1.compareTo(t2);
        } else {
            return this.f.compare(t1, t2);
        }
    }

    public Nodo<E> padre(final Nodo<E> nodo) {
        Nodo<E> raizTmp = this.getRoot();
        final Stack<Nodo<E>> pila = new Stack<Nodo<E>>();
        pila.push(raizTmp);
    ...
    public int equilibrado(final Nodo<E> n) {
        int hIzq = 0;
        int hDer = 0;
    ...
    /*
     * public Nodo<E> rotacionCompuestaIzquierda(Nodo<E> nodo){ Nodo<E> nodoTmp =
     * nodo; //57
     * 
     ...
     * 
     * 
     * public Nodo<E> rotacionCompuestaDerecha(Nodo<E> nodo){ Nodo<E> nodoTmp =
     * nodo;
     * 
     ...
     * 
     * return nodoTmp; } public Nodo<E> rotacionSimpleIzquierda1(Nodo<E> nodo){
     * Nodo<E> nodoTmp = nodo;
     * 
     * nodo = nodoTmp.getRight(); //clone?? nodoTmp.setRight(nodo.getLeft());
     * 
     ...
     * 
     * 
     * public Nodo<E> rotacionSimpleDerecha1(Nodo<E> nodo){ Nodo<E> nodoTmp = nodo;
     * nodo = nodoTmp.getLeft();
     * 
     * nodoTmp.setLeft(nodo.getRight()); nodo.setRight(nodoTmp);
     * 
     * return nodo; }
     */
    public int extraeFactorE(final Nodo<E> nodo) {
        if (nodo != null) {
